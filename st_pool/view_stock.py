...
1.控制下载界面 html
'''
def down_stocke_data_ui(request):
...
'''
def one_stockeolddata_show(request):
    ...
    df = pd.read_csv(oldstockdatapath + code + '.csv', dtype=object,header=None)
    ...
    for index, row in df_new.iterrows():
            ...
            if ((datetime.datetime(int(y[0]), int(y[1]), int(y[2])) - datetime.datetime(int(x[0]), int(x[1]),
                ...
                data.append(xx)
    # print 'data='+ data
    code =['stock',str(stockcode)] # 不知道什么原因, 000172 被转成 233 之类的数字
    print  data

    # return render(request, 'fundui/onefundui.html', {'code': code, 'data':data})
    return render(request, 'stockui/onestockui.html', {'code': code, 'data':data})
...
def stock_old_all_show(request):
    stock_pool_path = BASE_DIR + '/st_pool/get_stock_data_2019/股票池.csv'
    # oldstockdatapath = BASE_DIR + '/st_pool/get_stock_data_2019/stock_old_data/data/'
    oldstockdatapath = BASE_DIR + '/st_pool/get_stock_data_2019/stock_old_data/data_2014_2019/'
