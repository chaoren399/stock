...
5 跟 K 线,  前 3 天一字板, 第 4 天涨停吧, 第 5 天缩量 非涨停
https://www.yuque.com/chaoren399/gm281w/txp5z1/

ChengJie1


第一天 涨停板, 第 2 天高开的阳线, 缩量, 非涨停板, 第 3 天涨停板.
...
def get_all_ChengJie1(localpath1):
    info1=  '--承接1start--   '
    ...
    for index, row in data.iterrows():
        ...
        len1 = len(data6_1)
        isAn_ChengJie1_model(data6_1, stock_code)
...
'''
def isAn_ChengJie1_model(data,stockcode):
    ...
    if(len_data >= 5):
        ...
        for index,row in data1.iterrows():
            if(index==0 and isZhangTingBan(row)==1 and isYiZiBan(row)==0): #涨停板 非一字板
                count=count+1
...
测试老师的案例
'''
def test_isAn_ChengJie1_laoshi():
    # 案例 1  君正集团 601216 20200728
    df1 = ts.pro_bar(ts_code='601216.SH',adj='qfq', start_date='20200206', end_date='20200727')
    data7_1 = df1.iloc[0:30]  # 前7行
    isAn_ChengJie1_model(data7_1,'601216.SH')

    # 案例 2  宝鼎科技 002552 20190930
    df1 = ts.pro_bar(ts_code='002552.SZ',adj='qfq', start_date='20190206', end_date='20190929')
    data7_1 = df1.iloc[0:30]  # 前7行
    isAn_ChengJie1_model(data7_1,'002552.SZ')
...
def test_isAn_DaYou_ziji():
    ...
    isAn_ChengJie1_model(data7_1,'002507.SZ')
...
def test_Befor_data():
    ...
    for index, row in data.iterrows():
        ...
        for i in range(0, len_1 -n + 1):
            ...
            isAn_ChengJie1_model(data7_4[i:i + 5], stock_code)
...
if __name__ == '__main__':
    ...
    # get_all_ChengJie1(localpath1)
    # test_isAn_ChengJie1_laoshi()
    test_Befor_data()
