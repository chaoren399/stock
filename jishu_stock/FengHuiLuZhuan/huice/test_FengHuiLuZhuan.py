...
from jishu_stock.Tool_jishu_stock import writeLog_to_txt, writeLog_to_txt_nocode, print1, \
    writeLog_to_txt_path_getcodename, isYangXian
...
def isAn_FengHuiLuZhuan_model(data,stockcode):
    ...
    if(len_data >= 24):
        ...
        key_3=0; # 第 3 天阳线的收盘价 高过 涨停板的收盘价
        ...
        for index,row in data1.iterrows():
            ...
            if(index==1 and isZhangTingBan(row)==1): #涨停板
            ...
            if(index==3 ):
                #第 3 天阳线的收盘价 高过 涨停板的收盘价
                day4_close= row['close']
                if(isYangXian(row)==1 and day4_close > day2_close):
                    key_3=1
...
def test_isAn_FengHuiLuZhuan_laoshi():
    ...
    df1 = ts.pro_bar(ts_code='000400.SZ',adj='qfq', start_date='20190206', end_date='20200302')
...
def test_Befor_data():
    ...
    for index, row in data.iterrows():
        ...
        for i in range(0, len_1 - n + 1):
            ...
            isAn_FengHuiLuZhuan_model(data7_4[i:i + n], stock_code)
