...
def is_G8_Mai2_model_pro_Pro(data, stock_code):
    week10_60s = []
    WeekMa60s=[]
    if (len(data) > 0):
        for index, row in data.iterrows():
            ...
            WeekMa60s.append(WeekMa60)
        data_len = len(data)

        week10_60s_1 = week10_60s[0:1]
        # print week10_60s_1
        week10_60s_2 = week10_60s[1:8]

        #2 判断 数组 是不是 由小到大 #  最新的在数组最前边
        #3   60 日周线必须是上升趋势

        if(is_small_to_big(week10_60s_2)==1 and  is_big_to_small(week10_60s_1)==1 ):

            if( is_big_to_small(WeekMa60s)==1): #   60 日周线必须是上升趋势
                info = stock_code + "-------------------G8买2 pro"

                path = BASE_DIR + '/jishu_stock/JieGuo/G8/9/' + 'G8M2-Pro-' + datetime.datetime.now().strftime(
                    '%Y-%m-%d') + '.txt'
