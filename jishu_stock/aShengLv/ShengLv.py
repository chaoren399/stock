...
def get_shouyi(stockcode, mairu_riqi, zhisunddian,zhiying_shouyilv):
    ...
    df1 = ts.pro_bar(ts_code=stockcode, start_date=mairu_riqi, end_date='20211204')
    ...
    if (len_df1 >0):
        ...
        codeinfo = {'stockcode': stockcode, 'chajia': chajia, 'chicang_tianshu': chicang_tianshu
                       ,'maichu_riqi':mairu_riqi,'mairu_riqi':mairu_riqi ,'key1':key1,
...
def jisuan_all_shouyilv(chenggongs, modelname, zhiying_shouyilv):
    ...
    for index, item in enumerate(chenggongs):
        ...
        zhiyingdian=item['zhiyingdian']
        item1= get_shouyi(stockcode, mairuriqi, zhiyingdian,zhiying_shouyilv)
        ...
        if(item1 is not None):
            ...
            if(mairu_jiage < 30 and mairu_jiage> 4):  # 目前只统计 小于 30 元的股票 大于 4 元的股票
                ...
                zongde_yingli = zongde_yingli + chajia
            else:
    ...
    info=info+ '总的盈利:'+str(zongde_yingli) +"\n"
    info=info+ '成功次数:'+str(chenggong_cishu) +'\n'
    info=info+ '失败次数:'+str(shibai_cishu) +'\n'
    info=info+ '未止盈止损的次数:'+str(weizhiyingsun_cishu) +'\n'

    info = info + '所有符合模型的个数:'+str(len_chengogns) +'\n'
    info = info + '符合模型 中数据为空的个数:'+str(datais_null) +'\n'
    info = info + '成功+失败+未止损盈:'+str(chenggong_cishu+shibai_cishu+weizhiyingsun_cishu) +'\n'
    info = info + '平均持仓天数:'+str(zongde_chicang_tianshu / len_chengogns) +'\n'
    info = info + '所需资金总额:'+str(zijinzonge) +'\n'
