...
'''
计算收益率, 需要知道,   止盈收益率 10% , 止损 收益率-5% 之前都是 以前低位止损, 这次不同
stockcode
mairuriqi
zhisunddian 
zhiyingdian
...
zhiying_shouyilv= 1.05,1.10  止盈收益率

zhisun_shouyilv=0.95(-5%)  0.9 (-10%)  止损收益率
'''

IS_YEAR=1  # 计算收益率 不能太快的锁

FenCang_DaXiao=2000 #分仓的金额 比如, 2万元 分 10 仓,那么 每仓就是 2000 元.

def get_shouyi_10_5(stockcode, mairu_riqi,zhiying_shouyilv,zhisun_shouyilv):
    if(IS_YEAR):
        # sleep(0.05)
        sleep(0.2)
    ...
    if (len_df1 >0):
        ...
        zhisundian = day1_open * zhisun_shouyilv # -5% 的收益率 止损
        count=0
        for index, row in df1.iterrows():
            openprice=row['open']
            ...
            if(count < len_df1) : # 截止到今天期间到达止盈点,止损点
                if(openprice <= zhisundian or closeprice <= zhisundian):#如果达到 止损点, 卖出
...
def jisuan_all_shouyilv_10_5(chenggongs, modelname, zhiying_shouyilv,zhisun_shouyilv):
    # sleep(20)
    ...
    for index, item in enumerate(chenggongs):
        ...
        item1= get_shouyi_10_5(stockcode, mairuriqi,zhiying_shouyilv,zhisun_shouyilv)
        if (index == 0):
    ...
    info="\n"+modelname+'--10-5--回测盈利信息 目标止盈率:'+str(zhiying_shouyilv)+'--止损:'+str(zhisun_shouyilv)+'----日期间隔:'+str(zuizao_riqi) +'--'+str(zuihou_riqi)+"\n"
